...
func NewHandlerMap(lg logs.Logger, enableMetric bool) (*HandlerMap, error) {
	return &HandlerMap{
		lg:               lg,
...
func (hm *HandlerMap) MarkMsgAsHandled(msg *xuperp2p.XuperMessage) {
	hm.lg.Trace("MarkMsgAsHandled ", "log_id", msg.GetHeader().GetLogid(),
...
func (hm *HandlerMap) HandleMessage(stream interface{}, msg *xuperp2p.XuperMessage) error {
	...
	hm.lg.Trace("HandlerMap receive msg", "log_id", msg.GetHeader().GetLogid(),
		"msgType", msg.GetHeader().GetType(), "checksum", msg.GetHeader().GetDataCheckSum())
	if ok := hm.IsMsgAsHandled(msg); ok {
		hm.lg.Trace("HandlerMap receive is handled", "log_id", msg.GetHeader().GetLogid(),
			"msgType", msg.GetHeader().GetType(), "checksum", msg.GetHeader().GetDataCheckSum())
		return nil
	}
