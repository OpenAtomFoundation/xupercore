...
func NewXuperMessage(version, bcName, lgid string, tp xuperp2p.XuperMessage_MessageType,
	msgInfo []byte, ep xuperp2p.XuperMessage_ErrorType) (*xuperp2p.XuperMessage, error) {
	...
	if lgid == "" {
		msg.Header.Logid = logs.GenLogId()
	} else {
