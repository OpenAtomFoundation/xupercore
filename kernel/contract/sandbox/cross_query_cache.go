...
func endorsorQuery(req *pb.EndorserRequest, ce *pb.CrossEndorsor) (*queryRes, error) {
	...
	if err != nil {
		//log.Error("endorsorQuery NewEndorsorClient error", "err", err.Error())
		return nil, err
	}
	defer conn.Close()
	...
	if err != nil {
		//log.Error("endorsorQuery EndorserCall error", "err", err.Error())
		return nil, err
	}
	res := &pb.CrossQueryResponse{}
	err = json.Unmarshal(endorsorRes.GetResponseData(), res)
	if err != nil {
		//log.Error("endorsorQuery Unmarshal error", "err", err)
		return nil, err
	}
	queryRes := &queryRes{
		queryRes: res,
