...
func NewLedgerKeeper(chainCtx *def.ChainCtx) *LedgerKeeper {
	...
	//if _, err := lk.p2pSvr.Register(lk.p2pSvr.NewSubscriber(nil, netPB.XuperMessage_GET_BLOCKIDS, lk.handleGetBlockIds, "", lk.log)); err != nil {
	//    return lk, err
	//}
	//if _, err := lk.p2pSvr.Register(lk.p2pSvr.NewSubscriber(nil, netPB.XuperMessage_GET_BLOCKS, lk.handleGetBlocks, "", lk.log)); err != nil {
	//    return lk, err
	//}
...
func (lk *LedgerKeeper) DoTruncateTask(utxovmLastID []byte) error {
	err := lk.ctx.Ledger.Truncate(utxovmLastID)
	if err != nil {
		return err
	}
	lk.syncTaskMg.ClearTask(lk.ctx.Ledger.GetMeta().GetTrunkHeight())
	return nil
}
 ...
 */
func (lk *LedgerKeeper) PutTask(ctx *TaskContext, action Status, targetHeight int64) {
...
func (lk *LedgerKeeper) updatePeerStatusMap() {
	...
	for peerID, _ := range state.RemotePeer {
		...
		lk.peersStatusMap.Store(peerID, true)
		lk.log.Trace("Init::", "id", peerID)
	}
}
...
func (lk *LedgerKeeper) handleSyncTask(lt *LedgerTask) error {
	...
	for nextLoop {
		...
		endFlag, blockIds, err := lk.getPeerBlockIds(headerBegin, HeaderSyncSize, peer[0])
		...
		if err == ErrTargetDataNotFound {
			...
			block, err := lk.ctx.Ledger.QueryBlock(headerBegin)
			if err != nil {
				return ErrInternal
			}
...
func (lk *LedgerKeeper) downloadPeerBlocks(headersList [][]byte) []*SimpleBlock {
	...
	for {
		...
		// cache并发读写操作时使用的锁
		ctx, cancel := context.WithTimeout(context.TODO(), SyncBlocksTimeout)
		defer cancel()
...
func (lk *LedgerKeeper) getBlocks(ctx context.Context, targetPeer string, blockIds [][]byte) (map[string]*SimpleBlock, error) {
	...
	msg := p2p.NewMessage(netPB.XuperMessage_GET_BLOCKS, req, p2p.WithBCName(lk.bcName))
	opts := []p2p.OptionFunc{
		p2p.WithAddresses([]string{targetPeer}),
	}
	res, err := lk.ctx.Net.SendMessageWithResponse(ctx, msg, opts...)
	if err != nil {
...
func (lk *LedgerKeeper) confirmBlocks(hd *global.XContext, blocksSlice []*SimpleBlock, endFlag bool) ([]byte, error) {
	...
	if listLen == 0 {
		...
		return lk.ctx.State.GetLatestBlockId(), ErrInternal
	}
	var err error
	var newBegin []byte
	...
	for ; index < listLen; index++ {
		...
		if _, err := lk.ctx.Ledger.QueryBlockHeader(block.internalBlock.GetBlockid()); err != nil {
			break
		}
	...
	if index == listLen {
		...
		return lk.ctx.State.GetLatestBlockId(), ErrTargetDataNotFound // 同上，脏读依旧没有关系
	}
	...
	if bytes.Compare(beginSimpleBlock.internalBlock.GetPreHash(), lk.ctx.Ledger.GetMeta().GetTipBlockid()) == 0 {
		...
		for ; index < listLen; index++ {
			...
			err := lk.ctx.State.PlayAndRepost(checkBlock.internalBlock.GetBlockid(), needRepost, false)
			...
			err = lk.ctx.Consensus.ProcessConfirmBlock(checkBlock.internalBlock)
			if err != nil {
	...
	for ; index < listLen; index++ {
		...
		if trunkSwitch {
			err := lk.ctx.State.Walk(checkBlock.internalBlock.GetBlockid(), false)
...
func (lk *LedgerKeeper) checkAndConfirm(needVerify bool, simpleBlock *SimpleBlock) (error, bool) {
	...
	// 如果已经存在，则立即返回
	if lk.ctx.Ledger.ExistBlock(block.GetBlockid()) {
	...
	// 区块加解密有效性检查
	if needVerify {
		if res, err := lk.ctx.Consensus.CheckMinerMatch(&pb.Header{Logid: simpleBlock.logid}, block); !res {
			lk.log.Warn("checkAndConfirm::check miner error", "logid", simpleBlock.logid, "error", err)
			return ErrServiceRefused, false
		}
	}
	cs := lk.ctx.Ledger.ConfirmBlock(block, false)
	if !cs.Succ {
