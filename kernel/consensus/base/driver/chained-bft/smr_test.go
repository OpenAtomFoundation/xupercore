...
var (
	NodePath = filepath.Join(utils.GetCurFileDir(), "/main/test")
...
func TestProcessProposalSingle(t *testing.T) {
	...
	s := NewSMR("node", log, p, t)
	// 相同proposalId不能重复提交
	err := s.ProcessProposal(1, []byte{0}, []string{NodeIp})
	if err != SameProposalNotify {
		t.Error("ProcessProposal error", "error", err)
	}
	err = s.ProcessProposal(1, []byte{1}, []string{NodeIp})
	if err != nil {
		t.Error("ProcessProposal error", "error", err)
	}
}
...
func TestSMR(t *testing.T) {
	...
	//      收集A
	//            收集B
	if sA.GetCurrentView() != 1 {
		t.Error("receive A ProcessProposal error", "view", sA.GetCurrentView())
		return
	}
	// 检查本地qcTree
