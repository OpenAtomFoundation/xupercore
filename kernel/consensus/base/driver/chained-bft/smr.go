...
func (s *Smr) handleReceivedProposal(msg *xuperp2p.XuperMessage) {
	...
	if _, ok := s.localProposal.LoadOrStore(utils.F(newProposalMsg.GetProposalId()), newProposalMsg.Timestamp); ok {
		return
	}
	...
	metrics.ConsensusMsgGauge.WithLabelValues(s.bcName, "proposal-fullvotes").Set(float64(0))
	time := (time.Now().UnixNano() - newProposalMsg.Timestamp) / int64(time.Millisecond)
	metrics.ConsensusMsgGauge.WithLabelValues(s.bcName, "proposal").Set(float64(time))
...
func (s *Smr) handleReceivedVoteMsg(msg *xuperp2p.XuperMessage) error {
	...
	if _, ok := s.localProposal.Load(utils.F(voteQC.GetProposalId())); !ok {
		...
		metrics.ConsensusMsgGauge.WithLabelValues(s.bcName, "vote-drop").Set(float64(1))
		return EmptyTarget
	} else {
		metrics.ConsensusMsgGauge.WithLabelValues(s.bcName, "vote-drop").Set(float64(0))
	}
	...
	if t, ok := s.localProposal.Load(utils.F(voteQC.GetProposalId())); ok {
		if proposalGetT, ok := t.(int64); ok && proposalGetT != 0 {
			...
			metrics.ConsensusMsgHistogram.WithLabelValues(s.bcName, "proposal-fullvotes").Observe(float64(time))
			metrics.ConsensusMsgGauge.WithLabelValues(s.bcName, "proposal-fullvotes").Set(float64(time))
		}
	}
	// 更新HighQC
...
// QueryNode 查询状态机QcTree结构中是否有指定QC挂上
func (s *Smr) QueryNode(inProposalId []byte) *ProposalNode {
	return s.qcTree.DFSQueryNode(inProposalId)
}
