...
func (pc *PluggableConsensus) CheckMinerMatch(consensusCtx cctx.ConsensusOperateCtx, block cctx.BlockInterface) (bool, error) {
	con := pc.getCurrentConsensusComponent()
	if con == nil {
		return false, EmptyConsensusListErr
	}
	return con.CheckMinerMatch(consensusCtx, block)
}
...
func (pc *PluggableConsensus) updateConsensus(contractCtx cctx.FakeKContext, height int64) error {
	...
	if height > pc.nextHeight {
		pc.ctx.BCtx.GetLog().Warn("Pluggable Consensus::updateConsensus::trigger height error! Use old one.", "pluggable height", pc.nextHeight, "trigger height", height)
	...
	if err != nil {
		pc.ctx.BCtx.GetLog().Warn("Pluggable Consensus::updateConsensus::parse consensus configuration error! Use old one.", "error", err.Error())
		return err
	}
	cfg.BeginBlockid = pc.ctx.Ledger.QueryBlockByHeight(height).GetBlockid()
	consensusItem, err := pc.makeConsensusItem(pc.ctx, cfg)
	if err != nil {
		pc.ctx.BCtx.GetLog().Warn("Pluggable Consensus::updateConsensus::make consensu item error! Use old one.", "error", err.Error())
		return err
	}
	transCon, ok := consensusItem.(ConsensusInterface)
	if !ok {
		pc.ctx.BCtx.GetLog().Warn("Pluggable Consensus::updateConsensus::consensus transfer error! Use old one.")
...
func NewPluggableConsensus(cCtx cctx.ConsensusCtx) (ConsensusInterface, error) {
	...
	if err != nil {
		pc.ctx.BCtx.GetLog().Error("Pluggable Consensus::NewPluggableConsensus::parse consensus configuration error!", "error", err.Error())
		return nil, err
	}
	cfg.BeginBlockid = pc.ctx.Ledger.GetGenesisBlock().GetBlockid()
	consensusItem, err := pc.makeConsensusItem(cCtx, cfg)
	if err != nil {
		pc.ctx.BCtx.GetLog().Error("Pluggable Consensus::NewPluggableConsensus::make first consensus item error!", "error", err.Error())
		return nil, err
	}
	transCon, ok := consensusItem.(ConsensusInterface)
	if !ok {
		pc.ctx.BCtx.GetLog().Error("Pluggable Consensus::NewPluggableConsensus::consensus transfer error!")
		return nil, BuildConsensusError
	}
	pc.stepConsensus.put(transCon)
	return pc, nil
}
...
func (pc *PluggableConsensus) makeConsensusItem(cCtx cctx.ConsensusCtx, cCfg cctx.ConsensusConfig) (base.ConsensusImplInterface, error) {
	if cCfg.ConsensusName == "" {
		cCtx.BCtx.GetLog().Error("Pluggable Consensus::makeConsensusItem::consensus name is empty")
		return nil, EmptyConsensusName
	}
	// 检查version是否相等, 原postUpdateConsensusActions公共部分
	specificCon, err := NewPluginConsensus(pc.ctx, cCfg)
	if err != nil {
		cCtx.BCtx.GetLog().Error("Pluggable Consensus::NewPluginConsensus error", "error", err)
		return nil, err
	}
