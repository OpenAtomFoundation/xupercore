...
func (c *Conn) newClient() (pb.P2PServiceClient, error) {
    ...
    if connState == "TRANSIENT_FAILURE" || connState == "SHUTDOWN" || connState == "Invalid-State" {
        c.log.Error("newClient conn state not ready", "state", connState, "id", c.id)
        c.Close()
        err := c.newConn()
        if err != nil {
            c.log.Error("newClient newGrpcConn error", "error", err.Error(), "id", c.id)
            return nil, err
        }
    }
...
func (p *ConnPool) GetAllConn() map[string]*Conn {
    allConn := make(map[string]*Conn, 128)
    p.pool.Range(func(key, value interface{}) bool {
        allConn[key.(string)] = value.(*Conn)
        return true
    })
