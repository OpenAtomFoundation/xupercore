...
const (
	...
	LabelConsensusPhase = "phase_tag"
	LabelBlockInfo      = "block_tag"
)

var DefBuckets = []float64{.001, .0025, .005, .01, .025, .05, .1, .25, .5, 1, 2.5}

var ConsDefBuckets = []float64{10, 50, 100, 250, 500, 1000, 2000, 3000}
...
var (
	...
	BlockGauge = prom.NewGaugeVec(
		prom.GaugeOpts{
			Namespace: Namespace,
			Subsystem: SubsystemLedger,
			Name:      "block_num",
			Help:      "Number of block to be truncated during rollback-op.",
		},
		[]string{LabelBCName, LabelBlockInfo})
	BlockCounter = prom.NewCounterVec(
		prom.CounterOpts{
			Namespace: Namespace,
			Subsystem: SubsystemLedger,
			Name:      "block_sum",
			Help:      "Sum of block truncated during rollback-op.",
		},
		[]string{LabelBCName, LabelBlockInfo})
)
...
var (
	...
	// consensus handling time
	ConsensusMsgHistogram = prom.NewHistogramVec(
		prom.HistogramOpts{
			Namespace: Namespace,
			Subsystem: SubsystemNetwork,
			Name:      "consensus_handle_seconds",
			Help:      "Histogram of consensus msg cost latency between peers.",
			Buckets:   ConsDefBuckets,
		},
		[]string{LabelBCName, LabelConsensusPhase})
	ConsensusMsgGauge = prom.NewGaugeVec(
		prom.GaugeOpts{
			Namespace: Namespace,
			Subsystem: SubsystemNetwork,
			Name:      "consensus_phase_data",
			Help:      "Max consensus msg cost latency between peers.",
		},
		[]string{LabelBCName, LabelConsensusPhase})
)
...
func RegisterMetrics() {
	...
	prom.MustRegister(BlockGauge)
	prom.MustRegister(BlockCounter)
	// state
	...
	prom.MustRegister(ConsensusMsgHistogram)
	prom.MustRegister(ConsensusMsgGauge)
}
