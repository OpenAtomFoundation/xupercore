...
import (
	...
	schnorr_ring_sign "github.com/xuperchain/xupercore/lib/crypto/client/schnorr/ringsign"
	schnorr_sign "github.com/xuperchain/xupercore/lib/crypto/client/schnorr/sign"
	"github.com/xuperchain/xupercore/lib/crypto/client/schnorr/verify"
...
func GetInstance() base.CryptoClient {
	schnorrCryptoClient := SchnorrCryptoClient{}
...
func (xcc SchnorrCryptoClient) CreateNewAccountWithMnemonic(language int, strength uint8) (*account.ECDSAAccount, error) {
	ecdsaAccount, err := account.CreateNewAccountWithMnemonic(language, strength, common.NistSN)
...
func (xcc SchnorrCryptoClient) CreateNewAccountAndSaveSecretKey(path string, language int, strength uint8, password string) (*account.ECDSAInfo, error) {
	ecdasaInfo, err := key.CreateAndSaveSecretKey(path, walletRand.SimplifiedChinese, account.StrengthHard, password, common.NistSN)
...
func (xcc SchnorrCryptoClient) ExportNewAccountWithMnemonic(path string, language int, strength uint8) error {
	err := account.ExportNewAccountWithMnemonic(path, language, strength, common.NistSN)
	return err
}
