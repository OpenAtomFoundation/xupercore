...
import (
	...
	"github.com/xuperchain/xupercore/lib/crypto/hdwallet/rand"
...
func Sign(keys []*ecdsa.PublicKey, privateKey *ecdsa.PrivateKey, message []byte) ([]byte, error) {
	...
	allOfE[(signerIndex+1)%lenOfRing] = new(big.Int).SetBytes(sm3.Sm3Sum(append(message, elliptic.Marshal(curve, x, y)...)))
	...
	for i := (signerIndex + 1) % lenOfRing; i != signerIndex; i = (i + 1) % lenOfRing {
		...
		allOfE[(i+1)%lenOfRing] = new(big.Int).SetBytes(sm3.Sm3Sum(append(message, elliptic.Marshal(curve, x, y)...)))
	}
	...
	//	x, y = curve.Add(x1, y1, x2, y2)
	//	newE := new(big.Int).SetBytes(sm3.Sm3Sum(append(message, elliptic.Marshal(curve, x, y)...)))
...
func Verify(keys []*ecdsa.PublicKey, signature, message []byte) (bool, error) {
	...
	for i := 0; i < lenOfRing; i++ {
		...
		x, y := curve.Add(x1, y1, x2, y2)
		e = new(big.Int).SetBytes(sm3.Sm3Sum(append(message, elliptic.Marshal(curve, x, y)...)))
	}
