...
func CreateNewAccountWithMnemonic(language int, strength uint8, cryptography uint8) (*account.ECDSAAccount, error) {
	...
	switch cryptography {
	case common.Nist: // NIST
		cryptographyBit = []byte{common.Nist}
	case common.Gm: // 国密
		cryptographyBit = []byte{common.Gm}
	default: // 不支持的密码学类型
...
func RetrieveAccountByMnemonic(mnemonic string, language int) (*account.ECDSAAccount, error) {
	...
	default: // 不支持的密码学类型同时还不是旧的助记词
		if isOldMnemonic == false {
			...
			err = fmt.Errorf("Only cryptoGraphy NIST[%d] and SM2[%d] is supported in this version, this cryptoGraphy is [%v].", common.Nist, common.Gm, cryptography)
			return nil, err
		}
